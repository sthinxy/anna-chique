    ...
    <div className="bg-foreground text-white">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2 px-4 py-1.5 text-xs md:px-8">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => speak("Bem vinda à Anna Chique. Aqui você compra vestidos e conjuntos no atacado. O preço é vinte e cinco reais por peça, e o pedido mínimo é doze peças.")}
            className="flex items-center gap-1 rounded-full bg-white/10 px-3 py-1 font-semibold hover:bg-white/20"
          ...
          >
            {textSize === "xl" ? <Minus className="h-3.5 w-3.5" /> : <Plus className="h-3.5 w-3.5" />}
            Texto
          </button>
          <button
            onClick={toggleEasy}
            className={`flex items-center gap-1 rounded-full px-3 py-1 font-semibold ${easyMode ? "bg-primary text-white" : "bg-white/10 hover:bg-white/20"}`}
